...
public class CheckCurrentOrderController implements Initializable {
	...
	public void switchBackToSceneDeliveryMan(ActionEvent event) throws IOException {
	...
	public void addContacts(ObservableList<Contacts> dataListRemove) {
		...
		ListStore.ListStore7.addAll(dataListRemove);
		
	}

	
	@FXML
	private void sendToScene9Action(ActionEvent event) throws IOException {
		ObservableList<Contacts> dataListRemove = FXCollections.observableArrayList();
		
		for(Contacts bean : ListStore.ListStore7) {
			if(bean.getSelect().isSelected()) {
				dataListRemove.add(bean);
			}
		}
		ListStore.ListStore7.removeAll(dataListRemove);
		
		FXMLLoader loader = new FXMLLoader(getClass().getResource("SceneCheckHistoryOrder.fxml"));
		root = loader.load();
		
		CheckHistoryOrderController Scene9c = loader.getController();
		Scene9c.addContacts(dataListRemove);
...
//		ListStore.ListStore7.removeAll(ListStore.ListStore7);
//	}
...
//		list4.addAll(ListStore.ListStore7);
...
//		list4.addAll(new Contacts("0003", "Customer 3", "12:45", "雞排,薯條"));
		orderView.setItems(ListStore.ListStore7);
	}
