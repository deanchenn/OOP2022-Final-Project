...
public class CheckHistoryOrderController implements Initializable {
	...
	public void addContacts(ObservableList<Contacts> dataListRemove) {
		...
		ListStore.ListStore9.addAll(dataListRemove);
	...
	private void deleteSelectedRows(ActionEvent event) {
		...
		for(Contacts bean : ListStore.ListStore9) {
			if(bean.getSelect().isSelected()) {
				dataListRemove.add(bean);
			}
		}
		ListStore.ListStore9.removeAll(dataListRemove);
	}
...
//	}
	...
	private void loadData() {
		...
		orderView.setItems(ListStore.ListStore9);
	}
}
