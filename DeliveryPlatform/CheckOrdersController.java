...
public class CheckOrdersController implements Initializable {
	...
	public void switchBackToScene(ActionEvent event) throws IOException {
	...
	public void addContacts(ObservableList<Contacts> dataListRemove) {
		...
		ListStore.ListStore8.addAll(dataListRemove);
	...
	private void deleteSelectedRows(ActionEvent event) {
		...
		for(Contacts bean : ListStore.ListStore8) {
			if(bean.getSelect().isSelected()) {
				dataListRemove.add(bean);
			}
		}
		ListStore.ListStore8.removeAll(dataListRemove);
	}
	
	@FXML
	private void sendToScene7Action(ActionEvent event) throws IOException {
		ObservableList<Contacts> dataListRemove = FXCollections.observableArrayList();
		...
		FXMLLoader loader = new FXMLLoader(getClass().getResource("SceneCheckCurrentOrder.fxml"));
		root = loader.load();
		
		CheckCurrentOrderController scene7c = loader.getController();
		scene7c.addContacts(dataListRemove);
...
//	}
	...
	private void loadData() {
		...
		orderView.setItems(ListStore.ListStore8);
	}
}
