...
public class Scene14Controller implements Initializable {
	...
	public void addContacts(ObservableList<Contacts> dataListRemove) {
		...
		ListStore.ListStore14.addAll(dataListRemove);
	...
	private void deleteSelectedRows(ActionEvent event) {
		...
		for(Contacts bean : ListStore.ListStore14) {
			if(bean.getSelect().isSelected()) {
				dataListRemove.add(bean);
			}
		}
		ListStore.ListStore14.removeAll(dataListRemove);
	}
...
//	}
	...
	private void loadData() {
		...
		orderView.setItems(ListStore.ListStore14);
	}
}
