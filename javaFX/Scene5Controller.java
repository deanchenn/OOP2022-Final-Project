...
public class Scene5Controller implements Initializable {
	...
	public void addContacts(ObservableList<Contacts> dataListRemove) {
		...
		ListStore.ListStore5.addAll(dataListRemove);
	...
	private void deleteSelectedRows(ActionEvent event) {
		...
		for(Contacts bean : ListStore.ListStore5) {
			if(bean.getSelect().isSelected()) {
				dataListRemove.add(bean);
			}
		}
		ListStore.ListStore5.removeAll(dataListRemove);
	}
	
	@FXML
	private void sendToScene4Action(ActionEvent event) throws IOException {
		ObservableList<Contacts> dataListRemove = FXCollections.observableArrayList();
		...
		Scene4Controller scene4c = loader.getController();
		scene4c.addContacts(dataListRemove);
...
//	}
	...
	private void loadData() {
		...
		orderView.setItems(ListStore.ListStore5);
	}
}
